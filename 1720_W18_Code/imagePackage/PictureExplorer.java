 ...
 * image and provide some basic functionality for exploration. This
 * functionality includes zooming in and out on the raster image and
 * pointing-and-clicking on the specific pixels of the raster image. When a
 * pixel is selected, this app provides information about it, including the
 * pixel's (x, y) coordinates, the pixels colour components (its red, green, and
 * blue values). The app also allows the user to select a particular pixel by
 * specifying a pair of (x,y) coordinate values.
 * 
 * Originally created for the Jython Environment for Students (JES). Modified to
 * work with DrJava by Barbara Ericson. Modified to work specifically with a
 * RasterImage by M. Baljko.
 * 
...
public class PictureExplorer implements MouseMotionListener, ActionListener,
	...
	private void createWindow() {
		...
		pictureFrame.pack();
		// pictureFrame.show();
		// MB: changed from show() to setVisible(true), since it is equivalent
		// and allows us to avoid the use of a deprecated method
	...
	private void setUpNextAndPreviousButtons() {
		...
		// "previous index");
		Icon prevIcon = new ImageIcon(this.getClass().getResource(
				"leftArrow.gif"), "previous index");

		Icon nextIcon = new ImageIcon(this.getClass().getResource(
				"rightArrow.gif"), "next index");
	...
	public void checkScroll() {
		...
		if (zoomFactor > 1) {
			...
			int maxIndexX = (int) (picture.getWidth() * zoomFactor) - rectWidth
					- 1;
...
/**
 * Encapsulates a image displayer, which tracks the user's "current location"
 * and shows it using graphical feedback (approximating a "+" sign on the
 * graphical display). The imager displayer is used as a component of the
 * PictureExplorer.
 * 
 * Copyright Georgia Institute of Technology 2004
 * 
 * @author Barb Ericson ericson@cc.gatech.edu
 */
class ImageDisplay extends JPanel implements Scrollable {
